...
const article = $('.lorem-text');

form.onsubmit = (e) => {
    e.preventDefault();

    const value = parseInt(amount.value);
    const randNumb = Math.floor(Math.random() * text.length);

    if(isNaN(value) || value <= 0 || value > 9) {
        article.innerHTML = `<p>${text[randNumb]}</p>`;        
    }
    else {
        let tempText = text.slice(0, value);
        let tempArticle = tempText.map((article) => {
            return `<p>${article}</p>`;
        }).join("");
        article.innerHTML = tempArticle;
    }

}

//parseInt -> parse a string to an integer -> parseInt("string", "radix") -> radix: chuyển theo kiểu nào : binary, decimal, hexadecimal,....
// form -> check onsubmit
// isNaN() -> is a number(?)
// Create better lorem : https://hipsum.co/?paras=5&type=hipster-centric&start-with-lorem=1